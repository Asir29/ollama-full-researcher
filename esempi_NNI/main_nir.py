...
for Braille (encoded, with threshold=1) classification, for optimization through NNI.

NOTE: this version is a "fast prototyping" for initial NIR experiments.
...
experiment_name = "NIR_braille_th1" # name of the experiment as in the "main" script for NNI configuration
...
# Reset mechanism after a spike is emitted
parser.add_argument('-reset_mechanism',
                    type=str,
                    default="subtract",
                    help='Set whether the reset mechanism to be used is by subtraction ("subtract") or to zero ("zero")')
# Dealy of the spike emission
parser.add_argument('-reset_delay',
                    type=bool,
                    default=False,
                    help='Spike emission in a delayed time step or not')
...
#LOG.debug("------------------------------------------------------------------------------------")
LOG.debug("\n")
LOG.debug('NOTE that in this case the following settings are used: reset_mechanism="{}", reset_delay={}.\n'.format(settings["reset_mechanism"],settings["reset_delay"]))
...
def NNI_load_data(settings,device):
    ...
    ds_test = torch.load(os.path.join(dataDir,"Braille_th1/{}_collapsed_vowels_ds_test.pt".format(settings["experiment_data"])), map_location=device)

    ds_train = torch.load(os.path.join(dataDir,"Braille_th1/{}_collapsed_vowels_ds_train.pt".format(settings["experiment_data"])), map_location=device)
    ds_val = torch.load(os.path.join(dataDir,"Braille_th1/{}_collapsed_vowels_ds_val.pt".format(settings["experiment_data"])), map_location=device)
...
def NNI_build(settings, input_size, num_steps, device):
    ...
    class Net(nn.Module):
        def __init__(self):
            ...
            ##### Initialize layers #####
            ### Recurrent layer
            self.fc1 = nn.Linear(input_channels, num_hidden).to(device)
            self.fc1.__setattr__("bias",None) # for Xylo
            self.lif1 = snn.RSynaptic(alpha=settings["alpha_r"], beta=settings["beta_r"], linear_features=num_hidden, spike_grad=spike_grad, reset_mechanism=settings["reset_mechanism"], reset_delay=settings["reset_delay"]).to(device)
            ### Output layer
            self.fc2 = nn.Linear(num_hidden, num_outputs).to(device)
            self.lif2 = snn.Synaptic(alpha=settings["alpha_out"], beta=settings["beta_out"], spike_grad=spike_grad, reset_mechanism=settings["reset_mechanism"], reset_delay=settings["reset_delay"]).to(device)
